...
public class DevJwtConfig {
    ...
    SecretKey devJwtSecretKey(
        ...
        if (secretBytes.length < MINIMUM_SECRET_BYTES) {
            throw new IllegalStateException(
                    "DEV_JWT_SECRET debe tener al menos 32 caracteres"
            );
        }
    ...
    @Bean
    JwtEncoder devJwtEncoder(SecretKey devJwtSecretKey) {
        return NimbusJwtEncoder.withSecretKey(devJwtSecretKey).build();
    }
